...
function showSearch(info) {
    ...
    //if there is a list of gifs then runs a loop
        ...
        for (let a = 0; a < info.data.length; a++) { //loops through each gif and creates some html for each one
            ...
            gifHTML += `
            <div class="video-view" id="${info.data[a].id}" onclick = "copy('https://i.giphy.com/media/${info.data[a].id}/giphy.gif')">
    ...
    */
    ...
    for (let i = 0; i < info.data.length; i++) {
        document.getElementById(`${info.data[i].id}`).addEventListener('dblclick', function (e) {
            favGif(`${info.data[i].id}`);
            console.log('db')
        });
